#!/usr/bin/env python3
"""GSCSI scalper — firma OKX estilo PANDA. Limpia secrets. Prueba FLAG 1 y 0."""
...
def main():
    ...
    log.info(
        ...
        len(key), len(secret), len(passphrase), inst,
    )
    ...
    if len(closes) >= 15:
        ...
        rsi = None
    log.info("px=%.2f rsi=%s", price, ("%.1f" % rsi) if rsi is not None else "n/a")

    ok = None
    used = None
    for flag in ("1", "0"):
        data = call(key, secret, passphrase, flag, "/api/v5/account/balance")
        log.info("balance flag=%s code=%s msg=%s", flag, data.get("code"), data.get("msg"))
    ...
    if ok is None:
        log.error("Key rechazada en demo y en real. Revisa el Secret OKX_API_KEY (debe ser 32 chars, sin comillas).")
        log.info("PAPER publico sigue OK — rsi listo")
        return 0
