# main-fet.py — FET/USD OKX XPERP · cruce EMA7 x EMA21 · vela unica 30m · 2 contratos
import os
import time
import logging
...
# ==================== CONFIGURACIÓN ====================
BASE_ASSET  = 'FET'
AMOUNT      = 2.0             # 2 contratos por entrada (de golpe) — VERIFICAR nocional en 1a corrida
SL_PCT      = 0.010           # 1.0% SL
TP_PCT      = 0.015           # 1.5% TP
TD_MODE     = 'cross'
...
# ==================== DETONANTE: cruce EMA7 x EMA21 en UNA vela de 30m ====================
def evaluate_signal(symbol):
    """Evalua SOLO la ultima vela cerrada de 30m (cruce dentro de una sola vela)."""
    try:
...
def verify_setup():
    ...
    fet  = (bal.get('FET') or {}).get('free')
    usdc = (bal.get('USDC') or {}).get('free')
    log.info(f"Autenticacion OK | Colateral -> FET: {fet} | USDC: {usdc}")
...
def run_cycle():
    ...
    log.info(f"Senal confirmada: {signal} (vela {candle_ts}). Abriendo posicion de {AMOUNT} contratos...")
    try:
...
def run_once():
    log.info("Modo ciclo unico (GitHub Actions) | FET bot XPERP-30m, 2 contratos.")
    verify_setup()
    if TEST_MODE:
        catalog_fet()
        return
