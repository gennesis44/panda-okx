...
GitHub Actions + OKX read-only keys:
  --mode inspect  → solo endpoints publicos (keys opcionales)
  --mode stats    → positions-history (hace falta lectura privada)
  --mode trade    → BLOQUEADO si LIVE_TRADING no es true; con key read-only OKX rechaza
"""
...
def enrich(df: pd.DataFrame) -> pd.DataFrame:
    ...
    span200 = min(200, max(2, len(out)))
    out["ema200"] = ema(out["close"], span200)
...
def score_market(
    ...
    if len(exec_df) < 2 or len(struct_df) < 1 or len(bias_df) < 5:
        return Signal(side="FLAT", score=0.0, engine="none",
                      vetoes=["insufficient candles after confirm-filter"],
                      reasons=["fail-closed: not enough closed bars"])
    ...
    ema200_lookback = float(bias_df["ema200"].iloc[-5])
    bull_bias = b["close"] > b["ema200"] or (b["ema55"] > b["ema200"] and b["ema200"] >= ema200_lookback)
    bear_bias = b["close"] < b["ema200"] or (b["ema55"] < b["ema200"] and b["ema200"] <= ema200_lookback)
    if bull_bias:
        long_pts += 2.0
    ...
    if vetoes:
        can_long = can_analysis_long = can_short = False
    ...
    if can_long:
        ...
        if vetoes:
            reasons.insert(0, "VETOES bloquearon disparo")
...
def run_inspect(ox: OkxClient) -> Dict[str, Any]:
    ...
    sig = score_market(
        exec_df, struct_df, bias_df, ticker, funding, oi, weekly_pack,
        daily=daily_state,
        now=datetime.now(timezone.utc),
    )
    ...
    raw, n = position_contracts(equity, risk_pct, sig.price or float(exec_df["close"].iloc[-1]), ct_val, stop_pct=stop_for_size)
...
def run_trade(ox: OkxClient, report: Optional[dict] = None) -> dict:
    ...
    print("PROTECTOR: key read-only o trade se intenta SOLO tras set_leverage + place_order.")
    try:
