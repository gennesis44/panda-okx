...
# INSTRUMENTO: XPERP SUI/USD (vencimiento) — USDT PROHIBIDO (MiCA/EEE)
import os
import time
import logging
...
def catalog_sui():
    ...
    print("[" + time.strftime('%H:%M:%S') + "] --- CATALOGO SUI ---", flush=True)
...
def resolve_symbol():
    ...
    if sym:
        ...
        _log_contract_size(sym)
        return sym
    raise RuntimeError("No hay SUI/USD (XPERP o SWAP-USD) activo. USDT prohibido.")
...
def execute_order(side, symbol, ref_price, amount, candle_ts):
    ...
    log.info("SUI " + side + " ejecutada con SL/TP adjuntos. ordId: " +
...
def evaluate_signal(symbol):
    ...
    try:
        ...
        e15 = ema(df['close'], EMA_SLOW)
        price = exchange.fetch_ticker(symbol).get('last') or df['close'].iloc[-1]

        log.info("Precio: " + str(price) + " | 1H EMA3/15: " +
                 format(e3.iloc[-2], '.5f') + "/" + format(e15.iloc[-2], '.5f') +
                 " | EMA3 " + (">" if e3.iloc[-2] > e15.iloc[-2] else "<") + " EMA15 (estado)")
        ...
        for i_curr in SIGNAL_WINDOW:
            ...
            up = (e3.iloc[i_prev] <= e15.iloc[i_prev]
                  and e3.iloc[i_curr] > e15.iloc[i_curr])
            down = (e3.iloc[i_prev] >= e15.iloc[i_prev]
                    and e3.iloc[i_curr] < e15.iloc[i_curr])

            if up:
                log.info("Cruce ALCISTA EMA3/15 detectado. Senal LONG.")
                return 'LONG', price, candle_ts
            if down:
                log.info("Cruce BAJISTA EMA3/15 detectado. Senal SHORT.")
                return 'SHORT', price, candle_ts

        log.info("Sin cruce EMA3/15 en la ventana. Vigilando.")
    except Exception as e:
...
def run_cycle():
    ...
    log.info("Senal " + signal + " (cruce EMA3/15 1H). Abriendo " + str(AMOUNT) + " contrato...")
    try:
...
def run_once():
    log.info("Modo ciclo unico | SUI CATAMARAN EMA3/15 1H | LONG SL4/TP5.5 | SHORT SL3/TP4.5 (XPERP USD).")
    verify_setup()
    if TEST_MODE:
        catalog_sui()
        return
...
def main_loop():
    log.info("Bot " + BASE_ASSET + " CATAMARAN | 1H EMA3/15 | LONG SL4/TP5.5 · SHORT SL3/TP4.5 | " +
