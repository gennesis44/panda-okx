# main-sui.py — Ax2-D · 15m detonante + 4H brújula + cooldown · SL 1.5% / TP 2.5% · 1x · 2 contratos
...
# ==================== CONFIGURACIÓN (Ax2-D) ====================
BASE_ASSET  = 'SUI'
AMOUNT      = 2.0        # 2 contratos (unidad REAL la reporta OKX en el log de contrato)
...
FIXED_SYMBOL    = os.getenv('OKX_SUI_SYMBOL', 'SUI/USDT:USDT')
# Guardia anti-unidad: nocional maximo permitido por entrada (~2.5x objetivo $1.57 del README).
MAX_NOTIONAL_USD = _f(os.getenv('OKX_SUI_MAX_NOTIONAL', '4.0')) or 4.0
...
def _contract_meta(symbol, price=None):
    """ctVal + ccy reales. Linear (SUI): usd = ctVal * price. Inverse/quote (USD/USDT): usd = ctVal."""
...
def resolve_symbol():
    ...
    sym = pick_future()
    if not sym:
        raise RuntimeError("No hay SUI SWAP ni FUTURE activo en esta cuenta/host.")
...
def _log_maxsize(symbol):
    """Extra de la casa: capacidad de contratos segun OKX (informativo)."""
    try:
        inst = exchange.market(symbol)['id']
...
def run_cycle():
    ...
    # Ax3.1 orden: la posicion abierta se verifica ANTES de la capacidad.
    # Cualquier posicion viva (incluso parcial) espera su SL/TP — nunca se amplia.
    existing = get_position_contracts(symbol)
    if existing > 0:
        log.info(f"Posicion {existing} contratos. Esperando SL/TP.")
        return
...
def run_once():
    log.info("Modo ciclo unico (GitHub Actions) | SUI Ax2-D: 15m + gate 4H + cooldown 60m | Ax3 fail-closed.")
    verify_setup()
    if TEST_MODE:
        catalog_sui()
        return
...
def main_loop():
    log.info(f"Iniciando bot {BASE_ASSET} Ax2-D | {TIMEFRAME}+{TF_FILTER} | "
             f"SL {SL_PCT:.1%} / TP {TP_PCT:.1%} | cooldown {COOLDOWN_MIN}m | {HOST}")
    verify_setup()
    while True:
        try:
            run_cycle()
