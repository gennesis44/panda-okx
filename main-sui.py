...
# INSTRUMENTO: XPERP SUI/USD (vencimiento) — USDT/SWAP PROHIBIDO en esta cuenta (colateral no-USDT)
import os
import time
import logging
...
BASE_ASSET  = 'SUI'
AMOUNT      = 2.0        # 2 contratos = 2 SUI (~$1.48)
...
# Guardia anti-unidad: 2 contratos = 2 SUI (~$1.5). Tope $3.0 da holgura
# y veta cualquier unidad sorpresa (p.ej. 10 SUI/contrato = $7.4).
MAX_NOTIONAL_USD = _f(os.getenv('OKX_SUI_MAX_NOTIONAL', '3.0')) or 3.0
...
def _contract_meta(symbol, price=None):
    """ctVal + ccy reales. Linear (SUI): usd = ctVal * price. Inverse/USD-quoted: usd = ctVal."""
...
def resolve_symbol():
    ...
    if sym:
        ...
        _log_contract_size(sym)
        return sym
    raise RuntimeError("No hay SUI/USD (XPERP o SWAP-USD) activo. USDT prohibido en esta cuenta.")
...
def execute_order(side: str, symbol: str, ref_price: float, amount: float, candle_ts: int):
    ...
    log.info(f"SUI {side} ejecutada con SL/TP adjuntos. ordId: {d0.get('ordId')} | "
             f"SL: {sl} | TP: {tp}")
    return True
...
def run_once():
    log.info("Modo ciclo unico (GitHub Actions) | SUI Ax2-D + guardias (XPERP USD, USDT prohibido).")
    verify_setup()
    if TEST_MODE:
        catalog_sui()
        return
