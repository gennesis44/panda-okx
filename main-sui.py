# main-sui.py — SUI/USD OKX XPERP · cruce EMA7 x EMA21 en vela de 15m · gapless
import os
import time
import logging
...
def verify_setup():
    ...
    sui = (bal.get('SUI') or {}).get('free')
    log.info(f"Autenticacion OK | Colateral -> SUI: {sui}")
...
def run_once():
    log.info("Modo ciclo unico (GitHub Actions) | SUI bot XPERP-15m gapless.")
    verify_setup()
    run_cycle()
